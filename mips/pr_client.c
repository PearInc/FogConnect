...
void pr_socket_read_cb(struct bufferevent *bev, void *cb_arg)
{
	int    ret 					= 0;
	size_t len					= 0;
...
void fog_socket_event_cb(struct bufferevent *bev, short events, void *cb_arg)
{
    struct pr_user_data* user_data = (struct pr_user_data*)cb_arg;
...
void fog_socket_send_cb(struct bufferevent *bev, void *cb_arg)
{
	struct pr_user_data* user_data = (struct pr_user_data*)cb_arg;
	//event_base_loopbreak(user_data->base);
    fog_socket_event_cb(bev, BEV_EVENT_EOF, cb_arg);
...
{
/*
   convention_n 各个位的定义。可以看出定义了的分部都是由pr_conn函数的相关参数（int protocol, int connect_server）来确定。
   31    23    15    7     0
// |-----|-----|-----|-----|
                  |     |-------------->低4位（0～3）表示双方约定的传输协议。
...
*/
    ...
    {
        case 0:
            fog_event_setcb(pr_conn, pr_recv_data, pr_close_connect);
            break;
        //以下为示例，根据应用需要来决定。
        case 1:
    ...
    switch (events)
    {
        case FOG_EVENT_CONNECTED:
            if (user_data)
            {
                ...
                bufferevent_setcb(user_data->bev, pr_socket_read_cb, fog_socket_send_cb, fog_socket_event_cb, cb_arg);
	            bufferevent_enable(user_data->bev, EV_READ  | EV_PERSIST);
            }
